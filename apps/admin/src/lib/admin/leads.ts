...
// Mirrors the GET /api/leads (list) and /api/leads/[id] (detail)
...
// A row returned by GET /api/leads — a flattened lead joined with its
...
// The shape returned by GET /api/leads/[id]: the lead row joined with its
