...
// benign no-op. This helper NEVER throws to its caller: a per-send failure is
// caught, counted as `failed`, and (when the subscription is gone) reported via
// the optional `onGone` callback so the caller can prune it.

const logger = createLogger({
  service: 'admin:notifications:webpush',
...
export async function sendWebPush(
  ...
  // Not configured → no-op. Nothing was sent.
