...
<Tabs>
<Tab title="AWS web & admin">

Customer and admin apps run on AWS Lambda behind CloudFront through `sst.aws.Nextjs`:

1. Open GitHub Actions → **Deploy AWS** → **Run workflow**
2. Set `git_ref` to the last known-good tag or SHA
3. Run the workflow and verify both `/api/health` endpoints

A failed SST/Pulumi update can leave resources partially changed. Inspect the deployment logs and stack state, verify both `/api/health` endpoints, and redeploy the last known-good ref if production is unhealthy. A successful but bad release takes about 3–5 minutes to replace; use a PostHog flag first when it can contain the issue.

</Tab>
<Tab title="Render CMS">
...
| Alert | Source | Action |
| --- | --- | --- |
| Site down | BetterStack | Check CloudFront status and Lambda CloudWatch logs; verify Cloudflare DNS |
| API health failing | BetterStack | Check Lambda CloudWatch logs and Neon DB connection |
| Scheduled job missed | BetterStack heartbeat | Check QStash dashboard for delivery logs |
| New error spike | Sentry | Check error details, deploy fix or roll back |
...
| --- | --- | --- | --- |
| P0 | Site completely down | Immediate | CloudFront/Lambda unavailable, Cloudflare DNS outage, or Neon DB unreachable |
...
<Step title="Diagnose">
Check BetterStack and Sentry first, then the affected provider: CloudFront/Lambda in CloudWatch, Cloudflare DNS, Render CMS, or Neon.
</Step>
<Step title="Communicate">
Update the BetterStack status page with an incident message.
</Step>
<Step title="Fix or rollback">
...
| Bottleneck | Solution |
| --- | --- |
| Lambda cold starts or duration | Profile CloudWatch duration, collapse sequential DB calls, then cache hot reads in Upstash |
| CloudFront/static delivery | Inspect cache behaviour and asset headers before changing compute |
...
- [ ] Cloudflare DNS: `theroyalglow.in` / `www` resolve to the SST-managed web CloudFront distribution
- [ ] Cloudflare DNS: `admin.theroyalglow.in` resolves to the SST-managed admin CloudFront distribution
- [ ] Cloudflare DNS: `cms.theroyalglow.in` points to Render (`rgss-cms`)
- [ ] Cloudflare DNS: `docs.theroyalglow.in` points to Mintlify
- [ ] Cloudflare DNS: `status.theroyalglow.in` points to BetterStack
- [ ] TLS: verify ACM certificates on web/admin and provider-managed certificates on CMS/docs/status
- [ ] GMB: website field is `https://theroyalglow.in`
- [ ] GMB: booking action link is `https://theroyalglow.in/?book=1&utm_source=gmb`
