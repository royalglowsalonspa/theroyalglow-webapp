...
| `pprd` | Pre-production | `preprod` | + k6 load test + smoke suite + dependency audit |
| `prod` | Production | `main` | + manual approval → deploy + health check |

The `preprod` branch is **reset from `main` every 24h** via the Neon Branch Reset
...
PII. `pg_cron` jobs run on `main` only.
