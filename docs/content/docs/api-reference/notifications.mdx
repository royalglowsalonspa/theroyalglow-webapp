...
| --- | --- | --- | --- |
| `ids` | `string[]` | No | Notification ids to mark read (each non-empty). Omit to mark all unread read. |

<Tabs>
<Tab title="Mark specific">
...
| `keys.p256dh` | `string` | Yes | Non-empty client public key. |
| `keys.auth` | `string` | Yes | Non-empty client auth secret. |
...
| Name | Type | Required | Description |
| --- | --- | --- | --- |
| `endpoint` | `string` | Yes | Non-empty. The push endpoint to deactivate. |
