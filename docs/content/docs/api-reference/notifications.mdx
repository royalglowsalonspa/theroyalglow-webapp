...
<TypeTable
  type={{
    ids: {
      type: 'string[]',
      description: 'Notification ids to mark read (each non-empty). Omit to mark all unread read.',
      required: false,
    },
  }}
/>

<Tabs items={['Mark specific', 'Mark all read']}>
<Tab value="Mark specific">
...
Body fails `markReadSchema` (e.g. `ids` is not an array of non-empty strings).
...
<TypeTable
  type={{
    ...
    'keys.p256dh': {
      type: 'string',
      description: 'Non-empty client public key.',
      required: true,
    },
    'keys.auth': {
      type: 'string',
      description: 'Non-empty client auth secret.',
      required: true,
    },
  }}
/>
...
Body fails `pushSubscribeSchema` — e.g. `endpoint` is not a valid URL, or a key
is missing/empty. `details` carries field-level errors.
...
<TypeTable
  type={{
    endpoint: {
      type: 'string',
      description: 'Non-empty. The push endpoint to deactivate.',
      required: true,
    },
  }}
/>
...
`endpoint` is missing or not a non-empty string (raised as `badRequest`).
...
`ABLY_PRIVATE_KEY` is not configured (or the `ably` module is unavailable);
realtime is not yet provisioned.
