...
<Callout type="info">
  Money is an integer in **paise** (`pricePaise: 250000` = ₹2,500.00). Durations
  are whole minutes. All responses use the standard envelope
  `{ "success": true, "data": ... }`.
</Callout>
...
| Code | Status | When |
| --- | --- | --- |
| `INTERNAL_ERROR` | 500 | Unexpected/transient failure (e.g. database unreachable). `retryable: true`. |
...
| --- | --- | --- | --- | --- |
| `slug` | path | string | Required | The service's unique slug (e.g. `signature-haircut`). |
...
| Code | Status | When |
| --- | --- | --- |
| `NOT_FOUND` | 404 | No service exists for the given `slug`. |
...
| Parameter | In | Type | Required | Notes |
| --- | --- | --- | --- | --- |
| `date` | query | string | Required | Calendar date in `YYYY-MM-DD` format. Must be a real date and not earlier than today (compared in IST, UTC+5:30). |
...
| Code | Status | When |
| --- | --- | --- |
| `VALIDATION_ERROR` | 400 | `date` is missing, not `YYYY-MM-DD`, not a real calendar date, or in the past. |
| `INTERNAL_ERROR` | 500 | Unexpected/transient failure. `retryable: true`. |
