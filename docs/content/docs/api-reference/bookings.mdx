...
<TypeTable
  type={{
    branchId: {
      type: 'string',
      description: 'Non-empty. Branch must exist and have status operational.',
      required: true,
    },
    serviceType: {
      type: 'enum',
      description: 'salon or spa. All selected services must match this type.',
      required: true,
    },
    bookingDate: {
      type: 'string',
    ...
    serviceIds: {
      type: 'string[]',
      description: 'At least one non-empty id. De-duplicated server-side; every id must be an existing, active service of the matching serviceType.',
      required: true,
    },
    notes: {
      type: 'string',
    ...
    leadId: {
      type: 'string',
      description: 'Optional Meta-ad lead id for attribution.',
      required: false,
    },
  }}
/>
...
<TypeTable
  type={{
    reason: {
      type: 'string',
      description: 'Max 500 characters. Stored as the cancellation reason.',
      required: false,
    },
  }}
/>
