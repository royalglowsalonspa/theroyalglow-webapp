...
| --- | --- | --- | --- | --- |
| `allowedTypes` | `whitelist` | Yes |  | jpg, png, webp, pdf only (a whitelist, not a blacklist) |
| `maxSize` | `limit` |  | `10 MB` | Maximum size per file |
...
and `AISENSY_WEBHOOK_SECRET`. See [Environment Variables](/content/docs/environment-variables)
for the full list.
...
<Info>
  ...
  decided on. See [Authentication](/content/docs/authentication) for the onboarding flow.
</Info>
...
| --- | --- | --- | --- |
| `HttpOnly` | `true` | `true` | JavaScript cannot read the cookie (XSS cannot steal it) |
| `Secure` | `true` | `true` | Sent only over HTTPS |
| `SameSite` | `Lax` | `Lax` | Not sent on cross-site requests (CSRF defence) |
...
[Authentication](/content/docs/authentication) for the full sign-in design.
...
<Columns cols={2}>
  <Card title="Authentication" href="/content/docs/authentication">
