...
<Callout type="info">
  **In one line:** Security is layered — Zod validation, Upstash rate limits,
  nonce-based CSP, exact-origin CORS, file-type whitelisting, HMAC webhook
  verification, DPDP consent, six-role RBAC, and hardened session cookies all work
  together so one gap never becomes a breach.
</Callout>
...
<TypeTable
  type={{
    allowedTypes: { type: 'whitelist', description: 'jpg, png, webp, pdf only (a whitelist, not a blacklist)', required: true },
    maxSize: { type: 'limit', description: 'Maximum size per file', default: '10 MB' },
  }}
/>
...
<TypeTable
  type={{
    HttpOnly: { type: 'true', description: 'JavaScript cannot read the cookie (XSS cannot steal it)', default: 'true' },
    Secure: { type: 'true', description: 'Sent only over HTTPS', default: 'true' },
    SameSite: { type: 'Lax', description: 'Not sent on cross-site requests (CSRF defence)', default: 'Lax' },
  }}
/>
