...
<Tabs items={['Operational (expected)', 'Programmer (unexpected)']}>
<Tab value="Operational (expected)">
...
<Tab value="Programmer (unexpected)">
...
<Accordion title="UNAUTHENTICATED → redirect to sign-in">
The session is missing or expired — send the user to the sign-in page.
</Accordion>
<Accordion title="RATE_LIMITED → show a wait toast">
Show a toast: "Too many requests. Please wait."
</Accordion>
<Accordion title="VALIDATION_ERROR → let the form handle it">
Let the form surface field-level errors from `details`.
</Accordion>
<Accordion title="Anything else → show error.message in a toast">
Fall back to displaying `error.message` in a toast.
</Accordion>
</Accordions>
