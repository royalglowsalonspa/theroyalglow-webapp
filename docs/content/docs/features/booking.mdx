...
<Info>
  ...
  capture (see [CRM & Lead Pipeline](/content/docs/features/crm-leads)).
</Info>
...
<Info>
  **Walk-ins skip `pending`.** When a receptionist creates a walk-in booking the
  ...
  was no pre-booked slot to miss. See the [No-Show Policy](/content/docs/features/no-show-policy).
</Info>
...
`in_progress`. The cut-off is configurable — see
[Booking rules](/content/docs/features/booking-rules).
</Accordion>
...
completed. See [Loyalty — Gems](/content/docs/features/loyalty-gems).
</Accordion>
</AccordionGroup>
...
<Columns cols={2}>
  <Card title="Bookings API" href="/content/docs/api-reference/bookings">
  ...
  <Card title="No-Show Policy" href="/content/docs/features/no-show-policy">
  How no-shows are tracked and escalated.
</Card>
  <Card title="Booking Rules" href="/content/docs/features/booking-rules">
