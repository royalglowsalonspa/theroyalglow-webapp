...
- **First-touch attribution** — a customer's acquisition source (`organic`,
  `meta_ad`, `gmb`, `walkin`) is stored once and never overwritten by later
  visits.
- **Phone normalisation** — phone numbers are normalised to canonical
  `+91XXXXXXXXXX` before storage.
- **Public lead capture is rate-limited** — `POST /api/leads` is the only
  unauthenticated write endpoint and is rate-limited per IP.
- **Tags cap** — a customer can carry up to 10 tags; automatic tags cannot be
  removed by hand.
- **WhatsApp follow-up** — lead conversations are handled through AiSensy; when a
  status changes there, a webhook updates Neon so the database stays the single
  source of truth.
- **Manual leads** — leads created from inside the admin portal are forced to
  `source = manual` (versus `meta_ad` for ad traffic).

## Related links

<Cards>
  <Card title="Leads API" href="/docs/api-reference/leads" description="Public lead capture plus the admin pipeline (list, detail, status, notes)." />
  <Card title="Customers & CRM API" href="/docs/api-reference/customers-crm" description="Customer directory, profiles, notes, and tags." />
  <Card title="Booking System" href="/docs/features/booking" description="How a lead's booking links back for attribution." />
  <Card title="Data Model" href="/docs/data-model" description="The lead, lead_note, customer_tag, and customer_note tables." />
</Cards>
