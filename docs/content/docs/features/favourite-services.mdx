...
<TypeTable type={{
  user_id: { type: 'text', description: 'FK to user(id), ON DELETE CASCADE. Part of the composite primary key.', required: true },
  service_id: { type: 'text', description: 'FK to service(id), ON DELETE CASCADE. Part of the composite primary key.', required: true },
  created_at: { type: 'timestamptz', description: 'When the favourite was added — enables future "most recently favourited" sorting.', required: true, default: 'now()' }
}} />

## API Endpoints

<Tabs items={['GET', 'POST', 'DELETE']}>
<Tab value="GET">
