...
Redeploy the last known-good tag or SHA through the **Deploy AWS** workflow by setting its `git_ref` input. SST replaces the Lambda + CloudFront release in about 3–5 minutes. If a deploy fails, inspect SST/Pulumi state and both health endpoints because an update may be partially applied; redeploy the known-good ref when needed. Use a PostHog flag first when it can contain the issue.
