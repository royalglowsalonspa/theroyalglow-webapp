...
1. **Page mounts** — a `useEffect` opens an Ably connection using an
   `authCallback` that fetches a token from our own API.
2. **Token is issued** — `POST /api/ably/token` returns a short-lived, scoped
   Ably JWT for the signed-in user (see [Token Auth](#token-auth)).
3. **Subscribe** — the page subscribes to exactly the channels it is allowed to
   read and updates React state when a message arrives.
4. **Server publishes** — after a state-changing API route commits its database
   write, it publishes to every relevant channel using `ABLY_PRIVATE_KEY`.
5. **Page unmounts** — the connection is cleaned up. Ably handles reconnect on
   network loss and calls `authCallback` again when the token expires.
...
<Tabs items={["Customer", "Admin", "Staff"]}>
  <Tab value="Customer">
...
<Cards>
  <Card title="Notifications & Realtime API" href="/docs/api-reference/notifications" description="The POST /api/ably/token route and its scoped capabilities." />
  <Card title="Notifications & Email" href="/docs/features/notifications" description="Web Push and email — the other half of the communication layer." />
  <Card title="Background Jobs" href="/docs/background-jobs" description="Scheduled work that also drives realtime and push updates." />
  <Card title="Environment Variables" href="/docs/environment-variables" description="ABLY_PRIVATE_KEY and the rest of the realtime config." />
</Cards>
