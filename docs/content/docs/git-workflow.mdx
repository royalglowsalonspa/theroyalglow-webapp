...
<Info>
  **In one line:** Collaborative workflow with four persistent branches
...
The platform uses **4 persistent branches**, each mapped to an environment:
...
| Branch | Protection |
| --- | --- |
| `prod` | Ruleset protections, production security/load gates |
| `pprd` | Ruleset protections, integration and security/load gates |
| `test` | Ruleset protections, CI and integration/E2E gates |
| `dev` | Ruleset protections, CI and security gates |

GitHub Rulesets are the only merge gate: a reviewed pull request into `dev` that
satisfies the required status checks is merged directly. See
[Engineering Workflow](/engineering/index) and
[GitHub Rulesets](/github/rulesets) for the current policy.
...
🚀 Deploy apps/web + apps/admin to AWS through SST
```
...
perf: cache service catalog in Upstash Redis
...
<Tabs>
<Tab title="AWS (current)">

- `apps/web` and `apps/admin`: Lambda (ARM64) + CloudFront + S3 per app through `sst.aws.Nextjs`
- `deploy-aws.yml` on push to `prod`: GitHub OIDC → `bunx sst deploy --stage production`; when `AWS_DOMAINS_LIVE=true`, retry both public health endpoints up to 6 times with 15-second waits
- Recovery: inspect SST/Pulumi logs, stack state, and both applications, then manually deploy a known-good `git_ref` if needed
- Cloudflare remains authoritative DNS; SST manages only required validation and alias records

</Tab>
<Tab title="Other workloads">

- Payload CMS (`apps/cms`) remains on Render
- Invoice PDF service (`apps/invoicing`) remains on Cloud Run
- Neon, Upstash, QStash, Ably, Resend, and Cloudflare R2 remain unchanged
...
- [Testing](/content/docs/testing) — Full CI gate specifications
- [Deployment](/content/docs/deployment) — Platform configuration details
- [Environment Variables](/content/docs/environment-variables) — All secrets and their purpose
