...
| Branch | Protection |
| --- | --- |
| `prod` | Ruleset + Mergify protections, production security/load gates |
| `pprd` | Ruleset + Mergify protections, integration and security/load gates |
| `test` | Ruleset + Mergify protections, CI and integration/E2E gates |
| `dev` | Ruleset + Mergify protections, CI and security gates |

Mergify is the only merge orchestrator. Add the `merge-ready` label after
approval to enter its queue. See [Engineering Workflow](/engineering/index)
and [GitHub Rulesets](/github/rulesets) for the current policy.
