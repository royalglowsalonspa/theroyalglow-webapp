...
| Tool | Reason |
| --- | --- |
| **Cypress** | Slower than Playwright, no WebKit, worse Next.js App Router support |
| **Jest** | Vitest is drop-in replacement, 10x faster with Bun |
| **Chromatic** | Requires Storybook — RGSS doesn't use Storybook |
| **Datadog** | $31/host/mo — BetterStack + Sentry covers all needs at $0 |
| **Snyk** | Trivy + Socket.dev covers same ground for free |
...
Target: 50 concurrent users on preprod. Runs on every PR to `pprd`.
...
### On PR to `test`
- All above + Integration tests + Playwright E2E + Lighthouse CI

### On PR to `pprd`
- All above + k6 load test + OWASP ZAP + smoke test suite
...
Meticulous records real user sessions on preprod, then replays them as visual regression tests on every PR. Zero test code required — it learns from real usage.
