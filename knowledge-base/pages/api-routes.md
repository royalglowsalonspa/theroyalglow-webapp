...
|-------|--------|---------|------|-------|
| `/api/services` | GET | All active service categories and services, read directly from Neon through Drizzle. Returns: `{ categories: [{ id, name, services: [...] }] }` | No | No Redis cache |
| `/api/services/[slug]` | GET | Single active service detail by slug, read directly from Neon through Drizzle. Returns: `{ service: { id, name, slug, description, pricePaise, durationMin, category, gemsRedeemable, gemsRequired, image } }` | No | No Redis cache |
| `/api/availability` | GET | Generic 30-minute slot grid from Neon-backed business-hours settings. Query params: `date` (YYYY-MM-DD), `branchId`. Existing bookings, staff schedules, leave, and holidays are not wired yet; `branchId` is currently validated but otherwise unused. Returns: `{ slots: [{ startTime, endTime, available: boolean }] }` | No | No Redis cache |
...
| `/api/bookings/[id]/complete` | POST | Mark completed + checkout. Body: `{ paymentMethod, offerId? }`. Generates invoice, awards gems, emails PDF. | Receptionist | Invoice PDF generated by the Cloud Run invoicing service, stored in R2, emailed (Resend), gems awarded, Meta CAPI `Purchase`, Brevo attributes updated, Ably publish |
