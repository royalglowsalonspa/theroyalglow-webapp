...
- [x] 1. Relocate staff self-service APIs to `apps/admin/src/app/api/me/*`
  - [x] 1.1 Relocate the schedule GET endpoint
  ...
  - [x] 1.2 Relocate the leave GET + POST endpoint
  ...
  - [x] 1.3 Relocate the leave withdraw DELETE endpoint
  ...
  - [x]* 1.4 Write unit tests for the relocated `/api/me/*` behaviors
...
- [x] 2. Edit Admin RBAC to grant staff access to only its own surfaces
  - [x] 2.1 Add the `/me` route entry and Self-Service nav section in `apps/admin/src/lib/rbac.ts`
  ...
  - [x]* 2.2 Write property test: staff granted exactly the self-service routes
  ...
  - [x]* 2.3 Write property test: `/me` does not weaken the manager-level `/staff` route
  ...
  - [x]* 2.4 Write property test: self-service navigation visibility matches role level
  ...
  - [x]* 2.5 Write RBAC matrix example tests
...
- [x] 3. Checkpoint — APIs + RBAC
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Relocate staff self-service pages to `apps/admin/src/app/me/*`
  - [x] 4.1 Create the `/me` self-service layout
  ...
  - [x] 4.2 Relocate the schedule page
  ...
  - [x] 4.3 Relocate the leave page + client panel
  ...
  - [x]* 4.4 Write page-rendering tests for the schedule page
  ...
  - [x]* 4.5 Extend the admin route smoke test to cover `/me/*`
...
- [x] 5. Checkpoint — relocated pages render and gate correctly
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. Add the Web staff-redirect map and wire it into middleware
  - [x] 6.1 Create the pure `mapStaffRedirect` module
  ...
  - [x]* 6.2 Write property test: staff redirect preserves the deep-link sub-path
  ...
  - [x]* 6.3 Write property test: staff redirect preserves the query string verbatim
  ...
  - [x]* 6.4 Write property test: staff redirect is idempotent
  ...
  - [x]* 6.5 Write property test: staff redirect always targets the admin `/me` namespace
  ...
  - [x] 6.6 Wire `mapStaffRedirect` into `apps/web/src/middleware.ts`
  ...
  - [x]* 6.7 Write redirect mapping example tests
    - `/staff` → `…/me`; `/staff/schedule` → `…/me/schedule`;
      `/staff/leave?from=email` → `…/me/leave?from=email`
    - _Requirements: 9.5_

  - [x]* 6.8 Write middleware redirect ordering integration tests
...
- [x] 7. Clean up the Web staff surfaces
  - [x] 7.1 Delete the relocated web staff pages and APIs
  ...
  - [x] 7.2 Remove dead imports left by the deletion
...
- [x] 8. Static-invariant verification for Requirement 9
  ...
  - [x] 8.4 Run workspace typecheck and lint, fix any introduced errors
...
- [x] 9. Final checkpoint — full separation verified
  - Ensure all tests pass, ask the user if questions arise.
