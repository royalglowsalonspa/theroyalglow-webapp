...
The service catalogue is read-mostly and is a candidate for Cloudflare KV caching with a short TTL. This design keeps `/api/services` pure and side-effect-free so a cache layer can wrap it later without behavioral change. No caching is implemented in this feature.
