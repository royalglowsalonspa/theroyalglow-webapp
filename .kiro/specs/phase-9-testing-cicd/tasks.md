...
- [x] 6. GitHub Actions workflows + Lighthouse/k6/ZAP config
  ...
  - Create `.github/workflows/deploy-prod.yml`: `pre-deploy-checks` (gh api check-runs all success), `deploy` (`environment: production`, build via `bunx opennextjs-cloudflare build`, Sentry sourcemaps upload, `cloudflare/wrangler-action@v3` with `command: deploy` (workingDirectory `apps/web`), `bun run db:migrate` with `DATABASE_URL_UNPOOLED: secrets.DATABASE_URL_UNPOOLED_PROD`), `post-deploy` (health check retry 3×, smoke curls `/`,`/?book=1&utm_source=gmb`,`/services`,`/book`,`/api/health`, backup-exists check, BetterStack deploy/incident webhooks, auto-rollback via `wrangler rollback` on failure). `on: push: [prod]`
...
- Canonical env/secret names only (post-alignment): `DATABASE_URL`, `DATABASE_URL_UNPOOLED_PROD`, `UPSTASH_REDIS_REST_URL/TOKEN`, `NEXT_PUBLIC_R2_PUBLIC_URL`, `R2_ACCESS_KEY_ID/R2_SECRET_ACCESS_KEY/R2_ACCOUNT_ID`, `CLOUDFLARE_API_TOKEN/CLOUDFLARE_ACCOUNT_ID`, `SENTRY_AUTH_TOKEN`, `BETTER_STACK_HEARTBEAT_BACKUP`, `BETTER_STACK_DEPLOY_WEBHOOK`/`_INCIDENT_WEBHOOK`, `PPRD_URL`.
- Provisioning secrets / Cloudflare Workers / Neon API / BetterStack monitors is deploy-time ops; workflows reference the names. k6 against a live URL and the backup/replication crons are ops-activated.
